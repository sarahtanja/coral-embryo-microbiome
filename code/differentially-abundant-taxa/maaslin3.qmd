...
Here we aim to test changes in abundance (how many) and prevalence (presence/absence) of specific taxa due to stage, leachate, and their interaction. 
...
# Model 1: Categorical leachate & continuous time (interactive)
...
fit_Level7_Model1 <- maaslin3(
  input_data = ft_l7,
  input_metadata = meta,
  output = file.path(output_path, "Model1"),
...
```{r}
sig_m1 <- read_tsv(file.path(output_path, "Model1/significant_results.tsv"))
```
...
# Model 2: Categorical leachate & continuous time (additive)

**Model formula:**
`Abundance|Prevelance ~ leachate + hpf + (1|spawn_night)`
...
fit_Level7_Model2 <- maaslin3(
  input_data = ft_l7,
  input_metadata = meta,
  output = file.path(output_path, "Model2"),
  formula = ~ leachate + hpf + reads + (1|spawn_night),
  normalization = 'TSS',
  transform = 'LOG',
  correction = 'BH',
...
## significant results
```{r}
sig_m2 <- read_tsv(file.path(output_path, "Model2/significant_results.tsv"))
```
...
# Model 3: Categorical (interaction)
...
fit_Level7_Model3 <- maaslin3(
  input_data = ft_l7,
  input_metadata = meta,
  output = file.path(output_path,"Model3"),
...
## significant results
```{r}
sig_m3 <- read_tsv(file.path(output_path, "Model3/significant_results.tsv"))
```
...
fit_Level7_Model4 <- maaslin3(
  input_data = ft_l7,
  input_metadata = meta,
  output = file.path(output_path, "Model4"),
...
## significant results
```{r}
sig_m4 <- read_tsv(file.path(output_path, "Model4/significant_results.tsv"))
```
...
```
::: callout-note
No taxa were considered differentially abundant due to leachate in continuous additive model
:::
...
fit_Level7_Model5 <- maaslin3(
  input_data = ft_l7,
  input_metadata = meta,
  output = file.path(output_path, "Model5"),
  ...
  plot_summary_plot = TRUE,
  plot_associations = TRUE,
  heatmap_vars = c('leachate control',
                    'leachate low',
                    'leachate mid',
                    'leachate high'),
  verbosity = 'ERROR',
...
```{r}
sig_m5 <- read_tsv(file.path(output_path, "Model5/significant_results.tsv"))
```
...
```{r}
set.seed(1)
fit_Level7_Model6 <- maaslin3(
  input_data = ft_l7,
  input_metadata = meta,
  output = file.path(output_path, "Model6"),
...
# Model 7: Spawn night as fixed instead of random effect
