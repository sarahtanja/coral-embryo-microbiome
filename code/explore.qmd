...
Let's jump straight to significant results from the historical MaAsLin2 analysis. For current MaAsLin3 analysis, see [differentially-abundant-taxa/maaslin3.qmd](differentially-abundant-taxa/maaslin3.qmd).
...
::: {.callout-note}
## Understanding the Historical MaAsLin2 Model Results

The historical MaAsLin2 model uses an **additive design** (`leachate + timepoint`) without interaction terms. This means:
...
For a detailed explanation of the model design and how to interpret these results, see [differentially-abundant-taxa/maaslin2_interpretation.qmd](differentially-abundant-taxa/maaslin2_interpretation.qmd).

For current MaAsLin3 analysis with improved compositional data handling and multiple model specifications (additive, interactive, level contrasts, group-wise), see [differentially-abundant-taxa/maaslin3.qmd](differentially-abundant-taxa/maaslin3.qmd).

To gain more granularity:
1. Use MaAsLin3 with ordered factors for level contrasts
2. Add an interaction term (`leachate * hpf`) to test if effects differ across conditions
