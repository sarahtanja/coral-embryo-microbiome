...
```{r}
LeveneTest(shannon_df$Shannon ~ group, data = metadata) # homogeneity of variance
```
...
shannon_pvals <- summary(aov_shannon2)[[1]]
pval_leachate <- shannon_pvals["pvc_level", "Pr(>F)"]
pval_stage <- shannon_pvals["stage", "Pr(>F)"]
pval_interaction <- shannon_pvals["pvc_level:stage", "Pr(>F)"]
...
shan1 <- ggplot(shannon_df, aes(x = factor(leachate), y = Shannon, fill = factor(leachate))) +
  ...
  labs(
    ...
    fill = "Leachate",
    subtitle = paste0("Two-way ANOVA: ", format_pval(pval_leachate))
  ) +
  theme_classic() +
  theme(
...
shan2 <- ggplot(shannon_df, aes(x = factor(timepoint), y = Shannon, fill = factor(timepoint))) +
  ...
  labs(
    ...
    y = "Shannon Diversity Index",
    fill = "Timepoint",
    subtitle = paste0("Two-way ANOVA: ", format_pval(pval_stage))
  ) +
  theme_classic() +
  theme(
    plot.title = element_text(face = "bold", size = 14),
...
shan3 <- ggplot(shannon_df, aes(x = factor(timepoint), y = Shannon, fill = factor(leachate))) +
  ...
  labs(
    ...
    y = "Shannon Diversity Index",
    fill = "Leachate",
    subtitle = paste0("Interaction: ", format_pval(pval_interaction))
  ) +
  theme_classic() +
  theme(
    plot.title = element_text(face = "bold", size = 14),
