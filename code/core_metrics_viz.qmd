...
metadata <- metadata %>% 
  mutate(pvc_level=factor(leachate, levels = c(0,0.01,0.1,1)) )
...
```{r}
levels(shannon_df$pvc_level)
levels(shannon_df$stage)
```

```{r}
aov_shannon2 <- aov(Shannon ~ pvc_level * stage, data = shannon_df)
...
```{r}
class(shannon_df$leachate)
class(shannon_df$timepoint)
```
