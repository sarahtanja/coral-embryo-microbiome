...
-   Main effect of `leachate_mgL` (numerical continuous predictor; 0, 0.01, 0.1, 1 mg/L): Differences in the response between the levels of
    the `leachate_mgL` treatment, averaged over all `hpf`values. We choose
    to represent leachate in this model as a continuous variable
    here to look for linear patterns of
    response across increasing leachate concentration.
...
-   Interaction between `leachate_mgL` and `hpf`: Whether the slope of the
    response across hpf differs depending on the leachate category. This
    tests if `leachate_mgL` modifies how the response changes continuously
    over time; e.g., some leachate levels may accelerate or slow the
    effect of `hpf`.
