...
# Step 1: Load each QZA file
```{r}
ft <- read_qza("../salipante/241121_StonyCoral/270x200/250414_StonyCoral_270x200_featureTable_filtered.qza")$data
tax <- read_qza("../salipante/241121_StonyCoral/270x200/250414_270x200_representative-sequences_taxonomy.qza")$data
tree <- read_qza("../salipante/241121_StonyCoral/SEPP/insertion-tree.qza")$data
```
# Step 2: Load metadata
```{r}
metadata <- read_csv("../metadata/meta.csv")
```



# Step 3: Perform checks before integration
```{r}
str(ft)               # Check feature matrix
head(colnames(ft))    # Inspect sample IDs (colnames in matrix)

str(tax)             # Inspect taxonomy structure
rownames(metadata)    # Compare with sample names

# Optionally ensure consistent sample ordering
metadata <- metadata[colnames(ft), ]
```

# Step 4: Process taxonomy into a clean matrix
```{r}
# QIIME2 taxonomy strings are often semicolon-separated — split into ranks
tax_split <- do.call(rbind, strsplit(as.character(tax$Taxon), "; "))
rownames(tax_split) <- tax$Feature.ID
colnames(tax_split) <- c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")
```

# Step 5: Convert components to phyloseq-compatible objects
```{r}
OTU <- otu_table(as.matrix(ft), taxa_are_rows = TRUE)
...
table_path <- "../salipante/241121_StonyCoral/270x200/250414_StonyCoral_270x200_featureTable_filtered.qza"
taxonomy_path <- "../salipante/241121_StonyCoral/270x200/250414_270x200_representative-sequences_taxonomy.qza"
tree_path <- "../salipante/241121_StonyCoral/SEPP/insertion-tree.qza"
...
```{r}
feature_table <- qiime2R::read_qza(file.path(table_path))
```
...
```{r}
# quick sanity checks
if (any(is.na(ft_data))) stop("NA values found in the OTU matrix")
if (any(ft_data < 0)) stop("Negative counts found in the OTU matrix")
```
...
```{r}
setdiff(colnames(ft_data), meta$sample_id)
```
...
res_ancombc <- ancombc2(data = ps,
                        tax_level = "Family",
                        fix_formula = "stage * leachate",
                        rand_formula = "cross",
                        p_adj_method = "BH",
                        alpha = 0.05, 
