...
Core metrics include: - **Alpha Diversity**: Shannon diversity, Faith's
Phylogenetic Diversity, Evenness, Observed Features - **Beta
Diversity**: Bray-Curtis, Jaccard, Weighted UniFrac, Unweighted UniFrac
...
on Shannon diversity (Table X). Differences in Shannon diversity among
...
```{r}
aov_shannon2.num <- aov(Shannon ~ leachate * timepoint, data = shannon_df)
...
# Get mako colors for timepoints 4, 9, 14
mako_colors <- viridis::viridis(n = 3, option = "mako", begin = 0.2, end = 0.9)
...
shan3 <- ggplot(shannon_df, aes(x = factor(timepoint), y = Shannon, fill = factor(leachate))) +
  ...
  labs(
    ...
    y = "Shannon Diversity Index",
    fill = "Leachate",
    subtitle = paste0("Interaction: ", format_pval(pval_shan_intr))
  ) +
  theme_classic() +
  theme(
    plot.title = element_text(face = "bold", size = 14),
...
```{r}
aov_faith2 <- aov(Faith ~ pvc_level * stage, data = faith_df)
summary(aov_faith2)
```
...
```{r}
aov_faith2.num <- aov(Faith ~ leachate * timepoint, data = faith_df)
