...
::: callout-note
Check normality of Faith's Phylogenetic Diversity values using Shapiro-Wilk test. Since p > 0.05, we fail to reject the null hypothesis of normality, suggesting that Faith's PD values are approximately normally distributed and suitable for parametric testing.
:::
...
::: callout-note
Two-way ANOVA with Faith's Phylogenetic Diversity as the response variable and categorical factors (PVC leachate level and developmental stage) show that leachate concentration did not effect Faith's PD (*F*₃,₄₈ = 1.31, *p* = 0.280), while developmental stage did (*F*₂,₄₈ = 3.63, *p* = 0.034). However, the interaction between leachate and stage was not significant (*F*₆,₄₈ = 0.238, *p* = 0.962), indicating that the effect of leachate on Faith's PD did not differ across developmental stages.
:::
...
::: callout-note
Two-way ANOVA with Faith's Phylogenetic Diversity treating timepoint as a continuous variable showed similar results as categorical factors.
:::
...
```{r}
# Box plot by hpf
fait2 <- ggplot(faith_df, aes(x = factor(hpf), y = Faith, fill = factor(hpf))) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA) +
  geom_jitter(width = 0.2, size = 2, alpha = 0.6) +
...
```{r}
# Box plot by leachate x timepoint interaction
fait3 <- ggplot(faith_df, aes(x = factor(hpf), y = Faith, fill = factor(leachate))) +
