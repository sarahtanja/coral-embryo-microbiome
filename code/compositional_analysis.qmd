...
pca_plot_hpf <- ggplot(
  ...
  labs(
    title = "PCA Biplot by Stage, Leachate, and Parental cross",
...
pca_plot_hpf <- ggplot(pca_df, aes(x = PC1, y = PC2, color = stage, shape = leachate)) +
  geom_point(size = 3, alpha = 0.9) +
  labs(
    title = "PCA Biplot by Stage, Leachate, and Parental cross",
    x = paste0("PC1 (", round(variance_explained[1], 2), "% variance)"),
...
```{r}
# Additional PERMANOVA testing time as a continuous effect
